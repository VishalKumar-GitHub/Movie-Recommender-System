...
def save(rec: Recommender, path: str = "recommender.joblib") -> None:
    # Only persist columns that actually exist (future-proofing)
    cols = [c for c in SLIM_COLS if c in rec.df.columns]
    joblib.dump(
        {"df": rec.df[cols], "sim": rec.sim.astype(np.float32), "tfidf": rec.tfidf},
        path,
        compress=3,
    )
